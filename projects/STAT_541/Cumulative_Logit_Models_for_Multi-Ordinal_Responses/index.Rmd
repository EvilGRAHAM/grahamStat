...
$$
  L_{j} =
  \alpha_{j} + \sum_{i=1}^{p}\beta_{i}X_{i},
...
$$
  ...
  \frac{e^{\alpha_{j} + \sum_{i=1}^{p}\beta_{i}X_{i}}}{1 + e^{\alpha_{j} + \sum_{i=1}^{p}\beta_{i}X_{i}}}
$$
$$
  \implies
  \Prob\left( Y = j \right) =
...
$$
  =
  \frac{1}{1 + e^{\alpha_{j-1} + \sum_{i=1}^{p}\beta_{i}X_{i}}} - \frac{1}{1 + e^{\alpha_{j} + \sum_{i=1}^{p}\beta_{i}X_{i}}}
$$
...
http://grahamst.at/projects/STAT_541/Cumulative_Logit_Models_for_Multi-Ordinal_Responses/